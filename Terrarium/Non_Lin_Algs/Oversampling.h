...
class Oversampling
{
    ...
    void Upsample(const float* const* in, float** out,  size_t size)
    {
        for (size_t chan = 0; chan < NUM_CHANS; chan++)
        {
            ...
            for (size_t i = 0; i < size; i++)
            {
                ...
                for (int k = 0; k < Ndiv2; k += 2) // Every other sample
    ...
    void ClearBuff()
    {
        for (size_t chan = 0; chan < NUM_CHANS; chan++) 
            for(size_t i =0; i<47; i++){
                ring_buffer_[chan][i] = 0.0f;
                ring_buffer2_[chan][i] = 0.0f;
                
                if (i < 12)
                    ring_buffer3_[chan][i] = 0.0f;                
            }
    ...
    float ring_buffer_[NUM_CHANS][47];
    float ring_buffer2_[NUM_CHANS][47];
    float ring_buffer3_[NUM_CHANS][12]; // NDIV4 + 1

    float oversample_[NUM_CHANS][audio_block_size * 2];
    int position[NUM_CHANS];
